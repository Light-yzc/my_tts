...
class My_dit_stop_proj(GradientCheckpointingLayer):
    ...
    def forward(self, hidden_states: torch.Tensor, stop_labels: torch.Tensor, **kwargs):
        hidden_states = self.stop_proj(hidden_states)
        hidden_states = self.stop_actn(hidden_states)
        hidden_states = self.stop_head(hidden_states)
        loss = self.stop_loss(hidden_states, stop_labels)
        return loss

def My_dit_audio_proj(GradientCheckpointingLayer):
    def __init__(self, config: AceStepConfig):
        super().__init__()
        self.audio_proj = nn.Linear(config.audio_config.hidden_size, config.lm_config.hidden_size)
        self.audio_actn = nn.SiLU()
        self.audio_head = nn.Linear(config.lm_config.hidden_size, 2, bias=False)
        self.audio_loss = nn.CrossEntropyLoss(reduction="none")
    def forward(self, hidden_states: torch.Tensor, audio_labels: torch.Tensor, **kwargs):
        hidden_states = self.audio_proj(hidden_states)
        hidden_states = self.audio_actn(hidden_states)
        hidden_states = self.audio_head(hidden_states)
        loss = self.audio_loss(hidden_states, audio_labels)
        return loss
...
class AceStepConditionGenerationModel(AceStepPreTrainedModel):
    ...
    @classmethod
    def from_local(cls, pretrained_model_name_or_path: str, optimize: bool = True, lora_config=None, **kwargs):
        model = cls.from_pretrained(pretrained_model_name_or_path, **kwargs)
        if optimize:
            # Optimize the expensive part (decoder)
            model.decoder = torch.compile(model.decoder)
        
        # TODO: Implement LoRA loading if lora_config is provided
        if lora_config is not None:
             print("Warning: lora_config provided but LoRA integration is not yet implemented in AceStepConditionGenerationModel.from_local")
    ...
    def forward(
        ...
        # Encoder inputs
        # Text & Audio
        condition_hidden_states: torch.FloatTensor,
        ...
        condition_hidden_states = torch.where(full_cfg_condition_mask > 0, condition_hidden_states, self.null_condition_emb.expand_as(condition_hidden_states))
